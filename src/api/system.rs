...
async fn resolve_sandboxed_repo_path(state: &Arc<AppState>) -> String {
    ...
    select_repo_path(settings.sandboxed_repo_path, repo_path_from_env())
}
    ...
    if repo_path.exists() && !is_git_repo(repo_path).await {
        if repo_path.is_file() {
            ...
            tokio::fs::remove_dir_all(repo_path)
                .await
                .map_err(|e| format!("Failed to remove non-git directory at {}: {}", repo_path.display(), e))?;
        }
    }
...
#[cfg(test)]
mod tests {
    use super::{normalize_repo_path, select_repo_path};
